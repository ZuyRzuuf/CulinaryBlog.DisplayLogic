...
public class ArticleService : IArticleService
{
    ...
    /// <inheritdoc />
    public Article? GetArticleById(Guid id) => Articles.FirstOrDefault(article => article.Id == id);

    /// <inheritdoc />
    public List<Article> GetFilteredArticles(ArticleFilter? filters, ILogger logger)
    {
        var articles = Articles;

        if (filters == null)
        {
            logger.LogInformation("[DisplayLogic] No filters provided, returning all articles");
            return articles;
        };
        
        if (filters.Ids != null && filters.Ids.Any())
        {
            logger.LogInformation("[DisplayLogic] Filtering articles by ids: {Ids}", filters.Ids);
            articles = articles.Where(article => filters.Ids.Contains(article.Id)).ToList();
        }
        else if (filters.TagIds != null && filters.TagIds.Any())
        {
            logger.LogInformation("[DisplayLogic] Filtering articles by tag ids: {TagIds}", filters.TagIds);
        ...
        {
            logger.LogInformation("[DisplayLogic] Filtering articles by tag names: {TagNames}", filters.TagNames);
            articles = articles.Where(article => article.Tags.Any(tag => filters.TagNames.Contains(tag.Name))).ToList();
        }
        else if (filters.Id != null)
        {
            logger.LogInformation("[DisplayLogic] Filtering articles by id: {Id}", filters.Id);
